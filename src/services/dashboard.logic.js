export const buildDashboardSummary = projects => {
  ...
  const totalNilaiAktif = activeProjects.reduce(
    (a, b) => a + (Number(b.nilaiAnggaran) || 0), 0
  )

  const avgProgress = activeProjects.length === 0
    ? 0
    : Math.round(
        activeProjects.reduce((a, b) => a + (Number(b.progress) || 0), 0)
        / activeProjects.length
      )

  // Semua proyek perlu perhatian, tanpa batas
  ...
  const dangerProjects = activeProjects.filter(p => (p.progress || 0) < 30)

  const doneProjects = activeProjects.filter(p => (p.progress || 0) === 100)

  const lowestProgressProject = activeProjects
    .slice()
    .sort((a, b) => (a.progress || 0) - (b.progress || 0))[0]

  // Semua deadline terdekat, tanpa batas
  const nearestDeadline = activeProjects
    .filter(p => p.tanggalSelesai)
    .slice()
    .sort((a, b) => new Date(a.tanggalSelesai) - new Date(b.tanggalSelesai))
